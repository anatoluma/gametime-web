...
export default async function TeamsPage() {
  ...
  return (
    <main className="p-4 md:p-12 max-w-4xl mx-auto bg-[var(--surface)] min-h-screen">
        ...
        {list.map((team) => (
          ...
          >
            ...
            <div className="hidden sm:flex items-center gap-4">
               <span className="text-[11px] font-mono text-[var(--text-muted)]">{team.team_id}</span>
      ...
      {list.length === 0 && (
        <p className="p-20 text-center text-[var(--text-muted)] text-lg font-medium">{t("teams_empty")}</p>
      )}
    </main>
  );
}
